...
final class SchedulerTests: XCTestCase {
    ...
    func testOfDates() {
        ...
        let p = Scheduler.of(d0, d1, d2, d3)
        XCTAssertTrue(p.makeIterator().isAlmostEqual(to: ints, leeway: leeway))
    }

    func testDates() {
        let dates = Scheduler.of(1.days, 2.weeks).dates.makeIterator()
    ...
    func testNever() {
        XCTAssertNil(Scheduler.never.makeIterator().next())
    }

    func testConcat() {
        let p0: [Interval] = [1.second, 2.minutes, 3.hours]
        let p1: [Interval] = [4.days, 5.weeks]
        let p2 = Scheduler.of(p0).concat(Scheduler.of(p1))
        let p3 = Scheduler.of(p0 + p1)
    ...
    func testMerge() {
        ...
        let p0 = Scheduler.of(ints0).merge(Scheduler.of(ints1))
        let p1 = Scheduler.of(1.second, 1.second, 1.minutes, 1.seconds, 58.seconds, 1.hour)
        XCTAssertTrue(p0.isAlmostEqual(to: p1, leeway: leeway))
    }

    func testFirst() {
        var count = 10
        let p = Scheduler.every(1.second).first(count)
    ...
    func testUntil() {
        ...
        let p = Scheduler.every(1.second).until(until).dates
        let i = p.makeIterator()
    ...
    func testNow() {
        let p0 = Scheduler.now
        let p1 = Scheduler.of(Date())
        XCTAssertTrue(p0.isAlmostEqual(to: p1, leeway: leeway))
    }

    func testAt() {
        let p = Scheduler.at(Date() + 1.second)
    ...
    func testAfterAndRepeating() {
        let p0 = Scheduler.after(1.day, repeating: 1.hour).first(3)
        let p1 = Scheduler.of(1.day, 1.hour, 1.hour)
        XCTAssertTrue(p0.isAlmostEqual(to: p1, leeway: leeway))
    }

    func testEveryPeriod() {
        let p = Scheduler.every("1 year").first(10)
        var date = Date()
        for i in p.dates {
    ...
    func testEveryWeekday() {
        let p = Scheduler.every(.friday, .monday).at("11:11:00").first(5)
        for i in p.dates {
    ...
    func testEveryMonthday() {
        let p = Scheduler.every(.april(1), .october(1)).at(11, 11).first(5)
        for i in p.dates {
    ...
    func testOffset() {
        let p1 = Scheduler.after(1.second).first(100)
