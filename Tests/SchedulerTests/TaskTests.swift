...
final class TaskTests: XCTestCase {
    ...
    func testMetrics() {
        ...
        let task = Scheduler.after(0.01.second, repeating: 0.01.second).do(queue: .global()) {
            e.fulfill()
        }
    ...
    func testAfter() {
        ...
        let date = Date()
        let task = Scheduler.after(0.01.second).do {
    ...
    func testRepeat() {
        ...
        var count = 0
        let task = Scheduler.every(0.01.second).first(3).do(queue: .global()) {
            count += 1
    ...
    func testDispatchQueue() {
        ...
        let task = Scheduler.after(0.01.second).do(queue: q) {
    ...
    func testThread() {
        ...
        DispatchQueue.global().async {
            ...
            let task = Scheduler.after(0.01.second).do { task in
    ...
    func testReschedule() {
        ...
        DispatchQueue.global().async(after: 0.02.second) {
            task.reschedule(Scheduler.every(0.01.second).first(2))
        }
        waitForExpectations(timeout: 0.1)
    }
    ...
    func testAddAndRemoveActions() {
        ...
        let task = Scheduler.after(0.1.second).do {}
        let date = Date()
